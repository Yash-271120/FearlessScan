...
pub use volume::get_volumes;

pub const fn bytes_to_gb(bytes: u64) -> u16 {
    (bytes / (1e+9 as u64)) as u16
}

pub fn is_user_facing_volume(name: &str, mount_point: &str)-> bool{
    ...
    match platform {
        ...
        "linux" => {
            ...
            let system_paths = [
                "/mnt/wsl", "/mnt/wslg", "/mnt/wslg", "/snap",
                "/proc", "/sys", "/dev", "/run", "/tmp"
            ];

            !system_paths.iter().any(|p| mount_point.starts_with(p)) &&
            (mount_point == "/" || mount_point.starts_with("/mnt/") || mount_point.starts_with("/home"))
        }
        "macos" => {
